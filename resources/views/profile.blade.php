    ...
    @foreach ($activities as $activity)
        <div class="row mb-2">
            ...
            <div class="h-100 ml-3 my-auto col-xs-8">
                <p class="my-0">
                    <a href="{{ action('ProfileController@profile', $activity['subject']) }}">{{ $activity['subject']->name }}</a>
                    &nbsp;{{ $activity['action'] }}&nbsp;
                    @if ($activity['action'] == 'followed')
                        <a href="{{ action('ProfileController@profile', $activity['object']) }}">{{ $activity['object']->name }}</a>
                    @else
